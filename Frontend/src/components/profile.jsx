...
const Profile = () => {
  ...
  useEffect(() => {
    const getProfileData = async () => {
      try {
        const resp = await API.get("/api/profile-data");
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center px-4 sm:px-6 lg:px-8">
      <div className="bg-gray-800 p-6 sm:p-8 rounded-lg shadow-lg w-full max-w-lg">
        ...
        <button onClick={() => console.log('Edit Profile clicked')} className={`mt-6 w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg transition duration-300 transform hover:shadow-[0px_0px_7px_1px_#092139]  ${data?.length <= 0 ? "opacity-45 cursor-not-allowed" : ""}`} disabled={data?.length <= 0 ? true : false}>
          Edit Profile
        </button>
      </div>
    </div>
